...
Later legacy material refers to a “whole-page CRC32C checksum” for FSM pages and defines torn-page recovery, but it does not clearly establish one universal byte-range rule for all ordinary page types.

**Rewrite action:** Chapter 4 preserves the exact algorithm and staging rules that are actually specified and does not invent a universal coverage range. The owning later page/recovery passes should preserve any page-type-specific rules they contain.
