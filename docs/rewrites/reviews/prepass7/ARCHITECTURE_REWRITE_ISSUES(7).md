...
**Rewrite action:** in the next coherence-edit task, generalize the Chapter 4 summary so it does not imply UNIQUE indexes use a different physical key.

**Status:** no architecture decision required; editorial synchronization only.
...
**Rewrite action:** when Pass 9 migrates the complete RID-reuse protocol, explicitly cross-reference/gate whole heap-page recycling with the same safety rule. The early page-reuse wording must not be interpreted as an independent shortcut around RID epochs.

**Status:** no new architecture decision required if the later RID-reuse contract is preserved; cross-chapter synchronization is required.
