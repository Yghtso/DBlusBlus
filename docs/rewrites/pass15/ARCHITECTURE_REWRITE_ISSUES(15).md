...
**Core resolution:** V1 WAL now has explicit record-type codes 0..9, zero-only flags/reserved fields, semantic `total_length=48+payload_length`, zero external alignment, exact segment-tail padding, 16-byte WAL PageId, canonical PAGE_DELTA/IMAGE/INIT/MTR codecs, and explicit oversized-record rejection in Chapter 12.

**Status:** RESOLVED by the pre-Pass-10 core resolution.
...
**Core resolution:** `database.control` now has an exact 4096-byte slot codec with magic `DBLUSCTL`, version/header size, field offsets, zero flags/reserved bytes, whole-slot CRC32C, initial generation/state, no-wrap generation, alternating pwrite+fdatasync protocol, exact `next_file_id` semantics, and the status-reclaim cutoff.
...
**Core resolution:** Checkpoint identity is exactly CHECKPOINT_BEGIN LSN. BEGIN/DATA/END payloads, chunk indexes/count validation, installed begin/end/redo control fields, analysis start at BEGIN, redo start at checkpoint_redo_lsn (possibly earlier), and pre-BEGIN terminal-status redo are canonical in Chapter 13.
...
**Core resolution:** ReadEpochManager uses one mutex-protected monotonically increasing epoch. Readers capture current_epoch; retirement records current_epoch then increments it; RID reuse is safe exactly when no active reader epoch is <= retire_epoch. Recovered DEAD slots are conservatively re-enqueued.
...
**Core resolution:** Absolute status PageNos are preserved. `database.control.txn_status_reclaim_before` is a page-aligned durable cutoff; after advancing it durably, old whole status-page ranges may be hole-punched with keep-size sparse-file semantics. Lookup below cutoff returns runtime RETIRED without reading punched pages.
...
**Core resolution:** All persisted ordinary random-access pages are checksummed in WAL/recovery-enabled v1. BufferPool copies a stable page image under a read latch, finalizes CRC in the private image, establishes WAL-before-data, writes it, and clears dirty state only if modification_generation did not change.
...
**Core resolution:** Raw append remains a lower primitive, but WAL-mode page publication is serialized per file through extension + complete PAGE_INIT/BTREE_MTR append before published_page_count/reachability advances. Recovery reconstructs durable publications then truncates only the contiguous unpublished append tail. Heap publication and advisory FSM growth are explicitly separated.
...
**Core resolution:** V1 now uses an intrusive free-slot list: canonical UNUSED slots have zero coordinates and `aux=next_free`. Grace-complete reclamation compacts while DEAD, then atomically publishes UNUSED/free_slot_head. Vacuum exact index erase is idempotent (`EraseIfPresent`) across crash/retry.
...
**Core resolution:** READ COMMITTED internal statement retry is allowed only before the attempt's first persistent WAL-visible write. A retry-requiring conflict after any persistent attempt write aborts the transaction; a higher layer may rerun only in a new TxnId. This avoids abandoned-attempt writes becoming committed after CommandId advances.

**Status:** RESOLVED by the pre-Pass-10 core resolution.

**Pass 13 execution synchronization:** Physical UPDATE/DELETE target-spool revalidation now applies the resolved rule directly: discard/rebuild the spool with a fresh READ COMMITTED snapshot only if the attempt has made no persistent WAL-visible write. After the first persistent attempt write, a retry-requiring conflict aborts the transaction rather than replaying the statement under the same TxnId.

**Status:** RESOLVED and synchronized through execution.
...
**Core resolution:** Runtime terminal outcome, terminal state, and removal from snapshot-active membership now linearize together under transaction-registry synchronization. Terminal cache lookup precedes active lookup, and transaction logical locks are not released before that terminal publication point.
...
**Core resolution:** Every clean-to-dirty WAL-protected page transition now emits a full page image, making rec_lsn itself a reconstructible page-image LSN. The post-checkpoint FPI rule remains as an additional trigger. WAL retention through checkpoint_redo_lsn therefore preserves a full image for every dirty page.

**Status:** RESOLVED by the pre-Pass-10 core resolution.

## R-036 — Catalog bootstrap / `CATALOG_DATA` persistent representation is unspecified

**Type:** persistent catalog-bootstrap format gap discovered during Pass 10.

Legacy §§306–307 require a persistent relational system catalog plus minimal bootstrap metadata sufficient to locate/interpret it, but do not define the byte-exact bootstrap representation.

Earlier storage architecture reserves `FileKind::CATALOG` and `PageType::CATALOG_DATA` without defining whether/how those formats implement bootstrap.

Pass 10 deliberately does not invent bootstrap filename/object identity, fixed system-object IDs, CATALOG_DATA byte layout, bootstrap version/update protocol, or crash/durability publication rules.

**Rewrite action:** §16.9 locks only the minimal/self-hosting semantic boundary.

**Timing:** resolve before implementing a self-hosted persistent catalog; Pass 11 DDL may constrain publication but does not itself provide this byte format.

**Status:** OPEN.

**Pre-Pass-15 timing review:** No legacy optimizer section from §628 onward owns catalog bootstrap or `CATALOG_DATA`. Waiting for Pass 15 adds no architectural information. This gap is now classified **SOLVE BEFORE PASS 15 / FINAL OPTIMIZER COMPLETION** so Pass 16 can remain reconciliation rather than first-time persistent-format design.

**Pre-Pass-15 upper-core resolution:** V1 now uses one immutable database-root `catalog.dat` bootstrap file. It is exactly two pages: a CATALOG FileSuperblock (`object_id=0`) and PageNo 1 as a byte-exact `CATALOG_DATA` page with a 64-byte header and exactly six 32-byte entries for `sys_tables`, `sys_columns`, `sys_indexes`, `sys_index_columns`, `sys_constraints`, and `sys_statistics`. Bootstrap version/schema/generation, entry order, zero fields, checksum, identity validation, creation durability, and self-hosted catalog cross-check are canonical in §16.9.

The bootstrap page is never the ordinary catalog row store and is immutable after successful database creation.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-037 — Transactional allocation of catalog object IDs is unspecified

**Type:** catalog/DDL identity-allocation gap discovered during Pass 10.

Legacy §305 defines stable widths for TableId, ColumnId, IndexId, and TypeId, while the system catalog also contains identities such as `constraint_id`.

The source does not define how new TableIds/IndexIds/constraint identities are allocated, whether IDs are globally or per-kind monotonic, durable reservation/counter storage, crash-gap behavior, rollback reuse, or exact ColumnId allocation under schema evolution.

This is distinct from `next_file_id`, already owned by database-control metadata.

**Rewrite action:** Chapter 16 locks stable identity semantics but leaves allocation to transactional DDL.

**Timing:** Pass 11 / pre-DDL coherence resolution.

**Status:** OPEN, intentionally deferred to DDL ownership.

**Pass 11 canonical resolution:** V1 uses one durable database-wide uint64 `next_catalog_object_id` in `database.control` for TableId/IndexId/ConstraintId. Allocation advances that counter durably before returning the candidate, permits crash/abort gaps, never returns zero, and never reuses a value that may have reached persistent state. `ColumnId` remains table-local: CREATE TABLE assigns `1..N`, and future schema evolution uses a value greater than every historical ColumnId without reuse.

The control-slot v1 header is extended from 80 to 88 bytes with `next_catalog_object_id` at bytes `72..79`, CRC at `80..83`, and zero reserved suffix from byte 88.

**Status:** RESOLVED by Pass 11.

## R-038 — Built-in catalog TypeId numeric codes were absent from the legacy source

**Type:** resolved Pass-10 persistent-catalog completion.

`sys_columns` persistently stores `type_id`, but legacy §§305–313 define only the uint32 width and logical type names, not stable numeric built-in codes.

**Pass-10 completion:** §16.4 assigns `0` invalid and `1..7` to BOOLEAN, INT32, INT64, FLOAT64, DATE, TIMESTAMP, VARCHAR. NULL and UNKNOWN are non-persisted semantic pseudo-types.

**Status:** RESOLVED by the user-authorized Pass-10 architecture completion.

## R-039 — DATE/TIMESTAMP epoch and unit semantics were absent from the legacy source

**Type:** resolved Pass-10 logical/persistent semantic completion.

Earlier storage locks DATE as signed int32 and TIMESTAMP as signed int64 but intentionally leaves higher-level date/time meaning to SQL semantics.

Legacy §§312–322 name DATE/TIMESTAMP and cast support without defining epoch/unit/timezone semantics.

**Pass-10 completion:** §17.4 defines DATE as signed civil days from 1970-01-01 using the proleptic Gregorian calendar and TIMESTAMP as signed microseconds from 1970-01-01 00:00:00, timezone-naive in v1. Timezone-aware semantics and leap-second modeling remain deferred.

**Status:** RESOLVED by the user-authorized Pass-10 architecture completion.

## R-040 — Persistent default-expression encoding is not byte-exact

**Type:** persistent catalog semantic-format gap discovered during Pass 11.

Legacy §385 requires defaults to be stored as a bound/serializable catalog expression rather than raw SQL text only and permits constant or immutable scalar expressions.

The architecture does not yet define the versioned persistent expression-tree/blob grammar, stable persistent operator/function identity codes, literal/value encoding inside the blob, compatibility when registries evolve, or reopened-default validation.

**Rewrite action:** §21.12 preserves typed serializable default semantics without persisting ephemeral in-process implementation pointers/IDs by convention.

**Timing:** resolve before persistent nontrivial defaults / final cutover. An initial constant-only implementation would be compatible but does not define the eventual format.

**Status:** OPEN.

**Pre-Pass-15 timing review:** No §628+ optimizer material defines persistent default-expression serialization or stable persisted operator/function identities. This is an already-owned catalog/type gap and is classified **SOLVE BEFORE PASS 15**. A constant-only implementation remains compatible, but the architecture itself should close the general v1 format before final optimizer/cutover work.

**Pre-Pass-15 upper-core resolution:** V1 avoids a persistent expression-tree/function-ID compatibility layer entirely. Permitted defaults must be closed IMMUTABLE expressions. DDL fully evaluates/folds them once, coerces the result to the target type, and persists only that typed scalar result.

The byte-exact `DefaultValueBlob` has a 24-byte checksummed header and one shared §17.13 `PersistedScalarV1`; total blob size is capped at 4096 bytes. Runtime function/operator implementation IDs are not persisted. Original SQL text may be display metadata only.

Future expression-tree persistence, if needed, requires a new blob version.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-041 — Pass-12 execution-foundation precision completions

**Type:** resolved non-persistent execution-contract completion.

Legacy §§434–478 define the execution foundations strongly but leave several process-local details implicit.

Pass 12 completes them without creating a new persistent compatibility format:

```text
StringRef.prefix =
    first up to 4 bytes packed unsigned big-endian, zero-filled

dictionary normalization =
    one effective composed selection before hot evaluation

QueryArena =
    backing memory remains query-memory accounted

QueryExecutionContext =
    holds the execution ReadEpochGuard while candidate RIDs may be retained

PhysicalLimit early stop =
    pipeline-local safe stop, distinct from whole-query cancellation
```

Spill files remain temporary query formats and therefore do not acquire a database crash/reopen compatibility promise.

**Status:** RESOLVED by Pass 12.

## R-042 — Pass-13 execution semantic completions

**Type:** resolved execution-semantic completion.

Legacy §§479–567 define the practical execution algorithms but leave several cross-operator semantic details implicit.

Pass 13 makes the following v1 choices explicitly:
...
hash equality modes:
    ordinary join NULL non-match
    GROUP BY / DISTINCT NULL grouping identity

aggregate result signatures:
    COUNT -> INT64 NOT NULL
    SUM(INT32) -> INT64
    SUM(INT64) -> INT64
    SUM(FLOAT64) -> FLOAT64
    AVG(numeric) -> FLOAT64
    MIN/MAX -> input type for registered orderable types

sort prefix:
    order-preserving normalized prefix only

TopN:
    checked K = N + OFFSET
    final retained rows sorted before emission

DML target spool:
    each physical target RID at most once

RETURNING:
    no externally visible partial statement result before successful statement completion

FLOAT64 division:
    IEEE-754 binary64 result semantics
```

These are execution/runtime contracts rather than new persistent database formats.

**Status:** RESOLVED by Pass 13.

**Post-write synchronization:** Binder aggregate typing now explicitly consumes the same §29.3 signatures; Chapters 15/21 point to §31.9's successful-statement RETURNING publication boundary; Chapter 17 points to the §39.3.2 FLOAT64 arithmetic rule; parallel read workers consume an immutable transaction/snapshot/CommandId view.

## R-043 — Client-visible result-chunk lifetime completion

**Type:** resolved process/API lifetime completion.

Legacy §518 requires client-visible results to own or safely retain their memory but does not define a minimal cursor lifetime rule.

Pass 13 defines the synchronous baseline:

```text
ResultCursor::Next()
    -> owned/safely-retained client chunk

returned chunk remains valid until:
    next Next() call
    or cursor destruction
```

A caller needing a longer lifetime copies/materializes the result.

Internal borrowed DataChunks are never exposed directly across this boundary.

**Status:** RESOLVED by Pass 13.

## R-044 — `sys_statistics` persistent payload is semantic but not byte-exact

**Type:** persistent catalog/statistics format gap discovered during Pass 14.

Legacy §583 requires statistics to be stored in `sys_statistics` using an explicitly versioned payload containing at least:
...
Pass 14 additionally makes explicit the semantic need for width/index-correlation statistics.

However neither the legacy source nor already migrated catalog/type chapters define:

- exact payload header fields/offsets,
- stable persisted scalar `Value` encoding for MCV/min/max/histogram boundaries,
- array length/entry codecs,
- checksum/compression policy if any,
- forward/backward compatibility rules beyond "unsupported version -> rebuild".

Arbitrary C++ serialization is forbidden, so these details cannot be left to implementation convention.

**Rewrite action:** §34.14 locks the semantic/versioning contract and explicitly keeps the byte grammar open.

**Timing:** resolve before implementing persistent/reopenable nontrivial statistics or before final Pass-16 cutover. Statistics may remain rebuildable, so this does not threaten database correctness.

**Status:** OPEN.

**Pre-Pass-15 timing review:** Pass 14 now fully owns statistics semantics, and §628+ only consumes statistics for properties/search/cost quality. The statistics row/payload format will not become more defined by waiting. This is classified **SOLVE BEFORE PASS 15**, ideally together with one exact atomic StatsDescriptor publication/storage model.

**Pre-Pass-15 upper-core resolution:** `sys_statistics` now has an exact chunked persistence contract. A `StatsVersion` is `(TxnId, CommandId)`. TABLE/COLUMN/INDEX scope payloads use a common 40-byte checksummed `StatisticsPayloadV1` header and are split into at-most-4096-byte catalog fragments.

TABLE scope is the completeness manifest; COLUMN payloads use the shared persisted-scalar codec for min/max/MCV/histogram values; INDEX payloads are exactly 112 bytes and persist physical/live/invisible entry counts, leaf occupancy, and correlation. Missing/duplicate chunks, CRC/version/type violations, or incomplete manifests invalidate that statistics version without mixing versions.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-045 — Pass-14 optimizer/statistics semantic completions

**Type:** resolved non-correctness optimizer-contract completion.

Legacy §§568–627 define the estimator/cost architecture strongly but leave several ownership/interaction details implicit.

Pass 14 completes them coherently with the already migrated catalog/execution/index contracts:

```text
ANALYZE:
    one stable visibility snapshot
    complete immutable StatsDescriptor published atomically

IndexStatistics:
    explicit owner for leaf occupancy and leading-key/heap correlation

GROUP BY / DISTINCT estimates:
    nullable base columns include one NULL grouping class

B+ bound sentinels:
    transient search-only MIN/MAX key/RID objects
    never persisted/decoded as real RID values

nullable index search:
    IS NULL may be exact/sargable
    ordinary = NULL remains UNKNOWN/non-sargable
```

The exact numerical occupancy interpolation used when correlation statistics are absent remains a calibrated cost-model implementation detail rather than a correctness or persistent-format contract.

**Status:** RESOLVED by Pass 14.

**Pre-Pass-15 synchronization:** The earlier phrase “atomically publishes one immutable statistics descriptor version” is now precise: statement success may publish transaction-locally to the owning transaction, while global committed publication occurs only after terminal COMMITTED. R-046/R-044 own the exact persistent/runtime boundary.

## R-046 — ANALYZE is introduced by statistics architecture but is not integrated through the SQL/logical/physical execution stack

**Type:** cross-pass semantic/runtime integration gap discovered by the pre-Pass-15 review.
...
a v1 SQL maintenance command and requires a vectorized full-table statistics scan plus atomic publication of a complete immutable `StatsDescriptor`.

However the current rewritten upper stack still omits ANALYZE from:

```text
Chapter 18 initial parser/binder statement set
Chapter 20 logical operator family
Chapter 21 SQL-v1 supported target
Chapter 22 physical operator family
Chapter 31 maintenance execution operators
```

The current publication wording is also incomplete for explicit transactions: the global committed statistics/cache view must not publish a descriptor from an ANALYZE whose catalog writes later abort.

**Required resolution:**

Define one end-to-end v1 ANALYZE path covering at least:

```text
AstAnalyze / bound ANALYZE target
LogicalAnalyze
PhysicalAnalyze
maintenance execution through QueryMemoryManager
stable scan snapshot
transaction-owned sys_statistics write/update
stats_version generation
global cache publication only after terminal COMMITTED
rollback/failure leaves prior committed descriptor authoritative
modification-counter reset/publication boundary
```

ANALYZE should not require the target-table writer gate merely to obtain stable SQL rows; its MVCC snapshot supplies logical consistency. DDL/drop lifetime rules still protect the resolved descriptor/file ownership.

**Timing:** SOLVE NOW. Pass 15 consumes statistics but does not own the SQL/runtime path that produces them.

**Status:** OPEN.

**Pre-Pass-15 upper-core resolution:** ANALYZE is now first-class end to end:
...
-> LogicalAnalyze
-> PhysicalAnalyze
-> stable MVCC heap scan + bounded statistics collectors
-> transaction-owned sys_statistics rows
-> transaction-local descriptor after statement success
-> global immutable cache publication only after terminal COMMITTED
```

`ANALYZE table_name` is in the v1 parser/SQL surface. It does not acquire schema-changing DDL exclusivity or a writer gate merely to stabilize ordinary rows; MVCC supplies the logical snapshot. ABORT/cancel/incomplete payloads leave the previous committed descriptor authoritative.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-047 — Index access costing does not distinguish physical B+ entry pressure from logically live matches

**Type:** execution/statistics/cost-model coherence gap discovered by the pre-Pass-15 review.

The execution contract is explicit that every B+ entry is only a candidate RID and heap MVCC decides visibility.

Vacuum intentionally permits aborted/dead-version index garbage to remain until later cleanup.

Chapter 36 correctly says range-scan cost includes:
...
but `IndexStatistics` currently exposes only a generic:
...
without defining whether `entry_count` means:

```text
physical leaf entries
or
SQL-visible/live entries
```

or how index garbage pressure contributes to candidate/MVCC-reject cost.

A table may therefore have the same visible cardinality/selectivity but very different physical index work depending on vacuum state.

**Required resolution:**

Make the statistics/cost distinction explicit. A coherent v1 shape is to track/derive separately:

```text
physical_entry_count
leaf_page_count
average_entries_per_leaf
logical_live_entry_baseline / analyzed live rows
approximate invisible/garbage entry pressure
leading_key_heap_correlation
```

and let range/point costing estimate physical entries examined/candidate RIDs separately from visible output rows.

The exact garbage-distribution interpolation may remain approximate and calibrated; statistics still affect performance only.

**Timing:** SOLVE NOW. Pass 15 join/memo search will consume base access costs and should not be built on an ambiguous physical-entry definition.

**Status:** OPEN.

**Pre-Pass-15 upper-core resolution:** `IndexStatistics` now distinguishes:
...
Range/point access costing estimates SQL-visible result rows from live statistics and separately inflates physical B+ entries/candidate RIDs/MVCC work using physical-entry pressure. A fallback candidate-inflation ratio is canonical but explicitly approximate and capped; exact garbage distribution remains a calibrated cost-quality issue.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-048 — PredicateTruthEstimate is three-valued in shape but simple-predicate TRUE/FALSE/UNKNOWN mass is not fully specified

**Type:** cardinality-estimator semantic precision gap discovered by the pre-Pass-15 review.

Chapter 35 correctly introduces:
...
and requires SQL three-valued logic.

But some simple predicate rules specify only TRUE mass.

For example a non-NULL:

```sql
column = constant
```

uses MCV/residual TRUE selectivity, yet the architecture does not explicitly state that:

```text
unknown_fraction = column.null_fraction
false_fraction   = 1 - true_fraction - unknown_fraction
```

Likewise ordered range predicates and constant IN lists need exact UNKNOWN/FALSE completion before NOT/AND/OR can compose their truth-state estimates deterministically.

**Required resolution:**

Lock the basic truth triples for:

```text
non-NULL equality
range comparisons
IS NULL / IS NOT NULL
IN list with/without NULL list elements
```

and the independence fallback formulas for AND/OR.

The estimator may remain approximate, but every returned truth triple must be finite, clamped, normalized, and SQL-3VL coherent.

**Timing:** SOLVE NOW. Pass 15 depends on cardinality/selectivity but will not improve the primitive truth-state contract.

**Status:** OPEN.

**Pre-Pass-15 upper-core resolution:** Chapter 35 now specifies complete primitive `PredicateTruthEstimate` triples. Equality/range to a non-NULL constant assigns column NULL mass to UNKNOWN; IS NULL/IS NOT NULL have zero UNKNOWN; IN-list semantics distinguish lists with/without NULL; NOT swaps TRUE/FALSE; and independence fallbacks are exact under SQL 3VL:
...
All triples are finite, clamped, normalized, and TRUE alone drives filter cardinality.

**Status:** RESOLVED by the pre-Pass-15 upper-core resolution.

## R-049 — Pass-15 optimizer search/property precision completions

**Type:** resolved optimizer-search coherence completion.

Legacy §§628–725 define the property-aware System-R-style optimizer but leave a few interactions implicit.

Pass 15 completes them without changing the source's optimizer family:

```text
RequiredRowsObjective:
    kept separate from physical properties
    participates in memo/search identity wherever a finite requirement is propagated

physical operator availability:
    cost formulas do not make an unimplemented MergeJoin/SortAggregate/ordered-DISTINCT executable

large-join bounded search:
    deterministic greedy seed + bounded local improvement
    default four local-improvement passes

cost ties:
    relative epsilon default 1e-9
    collision-free canonical structural key decides ties
    FNV-1a PlanFingerprint remains diagnostic only

q-error zero handling:
    0/0 -> 1
    one-zero -> infinity marker

planning memory exhaustion:
    switches exhaustive DP to bounded heuristic
    then controlled OptimizerResourceLimit rather than arbitrary OOM
```

These are query-local optimizer/runtime contracts and create no persistent-format compatibility change.

**Status:** RESOLVED by Pass 15.
