...
Structural decoding/validation MUST reject a nonzero value in any of these reserved fields.
...
The B+ tree superblock stores at least:
...
Legacy §§109–179 do not define the complete byte offsets, widths, version fields, flag-bit semantics, or reserved-byte policy for the B+ tree-specific superblock metadata. Those details are tracked as an unresolved persistent-format issue rather than invented in this rewrite.
...
Before encoding FLOAT64:

- `-0.0` and `+0.0` are canonicalized to one value,
- all NaNs are canonicalized to one value.

The database total order is:
...
The source contract then requires a sortable IEEE-754 bit transform followed by big-endian encoding.

The SQL FLOAT64 comparison layer MUST use the same equality/order semantics.

Legacy §§109–179 do not define the exact canonical NaN payload bit pattern or the exact bit-transform formula. Those persistent-format details are tracked as an unresolved issue rather than inferred in this rewrite.
...
The B+ tree source contract does not assign persisted semantics to the slot `flags` field in §§109–179.

No flag values are invented by this rewrite.
...
The persisted meanings of the leaf-header `flags` field and reserved bytes are not assigned by legacy §§109–179 and are tracked with the other incomplete B+ page-format details.
...
The persisted meanings of internal-header `flags` and reserved bytes are not assigned in legacy §§109–179.
...
Legacy §§109–179 do not specify the exact byte offset/width/layout of `next_free_page_no` in a `BTREE_FREE` page; this is part of the unresolved B+ persistent-format issue.
...
### 8.19.3 Root metadata latch

Each tree has a small root-metadata latch protecting structurally sensitive metadata such as:

- root replacement,
- height changes,
- structurally necessary first/last leaf changes,
- free-list-head updates where needed.

Normal traversal holds this latch only long enough to safely obtain and pin the current root.

It MUST NOT be held for the duration of ordinary point operations.

### 8.19.4 Latch acquisition order
...
The exact expected B+ node format-version value, common-header `header_size` contract, node flag semantics, and reserved-byte rules are not fully specified by legacy §§109–179; they remain part of the persistent-format issue recorded for final resolution.
...
22. Vertical page-latch acquisition is parent-before-child.
23. Horizontal adjacent-leaf latch acquisition is left-to-right or the operation restarts.
24. Tree-local page reuse occurs only after safe structural detachment.
25. Physical UNIQUE-index duplicates remain representable because uniqueness is transactionally enforced above the tree.
---
...
B+ superblock-specific field offsets, B+ node flag/reserved semantics, BTREE_FREE layout, exact B+ node format-version/header-size rules, and exact FLOAT64 memcomparable bit encoding remain unresolved format details tracked in Appendix D / the rewrite issue register.
...
Current architecture-format issues relevant to the rewritten contract include:

1. **Ordinary-page checksum coverage.** The global exact byte range for every non-superblock checksummed page type remains unresolved; see rewrite issue R-010.
2. **B+ tree persistent metadata details.** Exact B+ superblock-specific offsets/widths, node/common format-version and header-size requirements, B+ node flag/reserved semantics, and the exact `BTREE_FREE` payload layout remain unspecified by legacy §§109–179; see R-014.
3. **FLOAT64 memcomparable index bytes.** V1 ordering/canonicalization semantics are locked, but the exact canonical NaN bits and sortable-bit transform formula are not byte-exact in the legacy contract; see R-015.
