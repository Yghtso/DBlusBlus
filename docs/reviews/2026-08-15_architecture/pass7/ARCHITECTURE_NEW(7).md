...
When the current in-memory reservation is exhausted:

1. calculate the next reservation boundary,
2. durably publish that boundary to the database control file,
3. only after the durable update succeeds may the allocator hand out IDs from the new reservation.

If the durable reservation update fails, IDs from the not-yet-durable range MUST NOT be handed out.
...
Legacy §§180–214 do not define whether `reserved_txn_id_end` is an inclusive last-reserved ID or an exclusive upper bound. That exact boundary convention remains an explicit architecture issue rather than being inferred here.
...
`active` contains normal transactions that were active for snapshot purposes at capture and satisfy:

```text
txn_id < snapshot.xmax
```

A transaction represented in this set remains invisible to this snapshot even if it commits later.
...
`xmin` is the lower transaction-ID horizon represented by the snapshot's active-set semantics.

If no transaction is relevant to that active-set horizon:

```text
xmin = xmax
```

The exact rule for whether `owner_txn_id` itself is stored in `active` and therefore participates in the stored `xmin` derivation is not made byte/algorithm exact by legacy §§180–214.

That distinction does not change the self-visibility algorithm because self is handled explicitly, but it affects vacuum-horizon precision and is therefore tracked as an explicit architecture question rather than guessed.
...
The persistent status state set is:
...
`IN_PROGRESS` is primarily runtime state represented by the active transaction registry; v1 does not require a durable status-page write at every `BEGIN`.

Two persisted bits are sufficient to represent four status states.

The exact persisted FileKind code, PageType code, two-bit numeric encoding, `RESERVED` state lifecycle semantics, and complete page-format validation contract are not assigned by legacy §§180–214 or later legacy transaction sections.

Those details remain an explicit persistent-format architecture issue.

## 9.12 Transaction-status page capacity

A transaction-status page uses exactly the normal 32-byte common page header and no additional page-specific header in the legacy v1 geometry.
...
```text
8160 * 4
= 32,640 transaction IDs
```

The transaction-status persistent mapping MUST be a deterministic pure function:
...
Legacy §§180–214 do not specify the exact arithmetic convention for the mapping, including whether reserved TxnIds `0` and `1` consume ordinary mapping positions.

That exact mapping is tracked with the transaction-status format issue rather than invented here.
...
`SELF` and `IN_PROGRESS` are lookup results, not additional persisted two-bit terminal-state codes.
...
9. One READ COMMITTED statement uses one stable snapshot per execution attempt.
10. REPEATABLE READ reuses one transaction snapshot and changes only its command boundary for later statements.
11. A transaction-status lookup never guesses that an unknown referenced normal TxnId committed.
12. A transaction is not published COMMITTED before its commit WAL is durable.
13. Read-only transactions may end without terminal persistent status because they create no persistent TxnId references.
14. Transaction-lifetime logical locks are released only after terminal outcome publication.
...
Current unresolved Pass-7 issues are:

1. **Transaction-status persistent format.** `txn_status.dat` has a locked page capacity/state set, but FileKind/PageType codes, two-bit numeric state codes, exact mapping arithmetic, and `RESERVED` lifecycle semantics are not byte-exact; see R-022.
2. **TxnId reservation boundary convention.** `reserved_txn_id_end` is durably required, but the legacy contract does not define inclusive versus exclusive end semantics; see R-023.
3. **Snapshot owner membership / xmin derivation.** snapshot self-visibility is exact, but whether `owner_txn_id` belongs in the stored `active` vector and exact `xmin` derivation is not locked; see R-024.
