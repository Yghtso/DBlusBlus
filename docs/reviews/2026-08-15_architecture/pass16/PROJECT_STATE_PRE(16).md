...
The current decoder still accepts nonzero reserved bytes; this remains the one open architecture question from the Phase 1 audit.
...
DecodeRid:
    bytes 14..15 are currently ignored
```

The Phase 1 audit identified this as the only unresolved architecture question.

A proposed strict v1 rule is:

```text
RID reserved bytes must be zero on write
RID reserved bytes must be zero on decode
```

This proposal has not yet been synchronized into `ARCHITECTURE.md`.

Existing database-produced RID encodings already use zero reserved bytes. A stricter decoder would affect only previously tolerated inputs containing nonzero reserved bytes.
