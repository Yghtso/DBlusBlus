...
| `72` | 4 | `crc32c` |
| `76` | 4 | `reserved32 = 0` |
| `80` | 4016 | reserved bytes, all zero |
|  | **4096** | **total slot** |
...
CRC32C is computed over exactly all 4096 slot bytes with bytes `72..75` logically zero.
...
9. unregister the transaction's own active snapshot(s)
10. finish transaction object cleanup
11. return success
```
...
7. unregister the transaction's own snapshots
8. finish transaction object cleanup
...
The transactional allocation policy for newly created TableIds/IndexIds and other catalog object IDs is owned by the transactional DDL/catalog-publication contract. It is not yet defined by the v1 architecture; see R-037.
...
A schema-changing DDL commit monotonically increases the table's current `schema_version`.
...
Cache keys use stable object IDs and names where name lookup is required.

Published descriptors are immutable snapshots.

A schema-changing operation invalidates/replaces affected cache entries; it does not mutate descriptor meaning behind active queries.
...
The transactional catalog-visibility/publication protocol is owned by the later DDL/catalog-visibility contract.
...
11. Bootstrap metadata remains minimal and cannot become a permanently divergent second catalog.
...
Current Pass-10 gaps intentionally left explicit are:
...
2. **Transactional catalog-object ID allocation.** Stable IDs are required, but allocation/durability/reuse policy for new TableId/IndexId/constraint identities is not specified by §§301–358 and belongs with DDL; see R-037.
